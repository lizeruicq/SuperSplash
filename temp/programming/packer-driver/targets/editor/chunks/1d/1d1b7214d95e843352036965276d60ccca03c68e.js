    ...
    execute: function () {
      ...
      _export("AIController", AIController = (_dec = ccclass('AIController'), _dec(_class = (_class2 = class AIController extends Component {
        ...
        freeDrive(ai, i) {
          ...
          const pos = ai.node.worldPosition; // 计算相对于Canvas中心的位置

          const relativeX = pos.x - this.canvasWorldPos.x;
          const relativeY = pos.y - this.canvasWorldPos.y; // 计算到边界的距离
          ...
          const nearTopBoundary = relativeY > this.sceneMaxY - margin; // 如果接近边界，开始边界转向（需要检查冷却时间）
          ...
          if (nearLeftBoundary || nearRightBoundary || nearBottomBoundary || nearTopBoundary) {
            ...
            if (this.boundaryTurnCooldowns[i] >= requiredCooldown) {
              ...
              // 在冷却时间内，继续正常行驶
              const requiredCooldown = this.boundaryTurnCooldowns[i] < 0 ? Math.abs(this.boundaryTurnCooldowns[i]) : 3.0; // console.log(`冷却中: ${this.boundaryTurnCooldowns[i].toFixed(1)}s/${requiredCooldown.toFixed(1)}s`);
        ...
        startBoundaryTurning(ai, i, nearLeft, nearRight, nearBottom, nearTop) {
          ...
          let targetAngle = 0;
          let turnDirection = 0;
          const currentAngle = ai.getCurrentAngle(); // 优先处理角落情况，避免判断混乱

          const isCorner = nearLeft && nearBottom || nearLeft && nearTop || nearRight && nearBottom || nearRight && nearTop;
        ...
        handleBoundaryTurning(ai, i, dt) {
          ...
          const startAngle = this.boundaryTurnStartAngles[i]; // 计算已经转向的角度
          ...
          if (Math.abs(angleTurned) >= 30) {
            ...
            const relativeX = pos.x - this.canvasWorldPos.x;
            const relativeY = pos.y - this.canvasWorldPos.y; // 如果已经转向足够角度且位置安全，停止转向
          ...
          const absAngleToTarget = Math.abs(angleToTarget); // 添加调试信息
...
//# sourceMappingURL=1d1b7214d95e843352036965276d60ccca03c68e.js.map