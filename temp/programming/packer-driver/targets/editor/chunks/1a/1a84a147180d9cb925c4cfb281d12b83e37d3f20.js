    ...
    execute: function () {
      ...
      _export("CameraFollow", CameraFollow = (_dec = ccclass('CameraFollow'), _dec2 = property(Node), _dec(_class = (_class2 = class CameraFollow extends Component {
        ...
        _detectMapSize() {
          ...
          const mapNode = this.node.parent.children.find(child => child.name === 'PlayGround');
        ...
        _clampToMapBounds(x, y, visibleWidth, visibleHeight) {
          ...
          const canvasWorldY = this.node.parent.worldPosition.y; // 地图中心为canvas中心
          ...
          let mapTop = canvasWorldY + this._mapHeight / 2; // 计算摄像机中心允许的移动范围
          // 确保摄像机可视范围不超出地图边界
        ...
        _updateCameraPosition(deltaTime, instant) {
          ...
          const deviceAspect = view.getVisibleSize().width / view.getVisibleSize().height; // 正确的可视范围计算方法
          // orthoHeight就是摄像机可视范围的完整高度

          const visibleHeight = camera.orthoHeight;
          const visibleWidth = visibleHeight * deviceAspect; // 使用正确的可视范围

          const usedVisibleWidth = visibleWidth;
          const usedVisibleHeight = visibleHeight; // clamp摄像机中心，保证可视范围始终在地图内
...
//# sourceMappingURL=1a84a147180d9cb925c4cfb281d12b83e37d3f20.js.map