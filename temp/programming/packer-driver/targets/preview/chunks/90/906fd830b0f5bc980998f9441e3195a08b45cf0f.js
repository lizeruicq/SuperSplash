    ...
    execute: function () {
      ...
      _export("CollisionDetector", CollisionDetector = (_dec = ccclass('CollisionDetector'), _dec2 = property(Node), _dec(_class = (_class2 = class CollisionDetector extends Component {
        ...
        _initCollisionSystem() {
          ...
          var mapInfo = this._tiledMap.getMapSize();
        ...
        isOnTrack(worldPos) {
          ...
          var mapPos = this._worldToMapPosition(worldPos); // 检查地图边界
          ...
          var tileGID = this._trackLayer.getTileGIDAt(mapPos.x, mapPos.y); // 如果tileGID为0，表示没有tile（不是赛道）
        ...
        _worldToMapPosition(worldPos) {
          // 获取地图节点的世界坐标
          var mapWorldPos = this.trackLayer.worldPosition; // 计算相对于地图的本地坐标

          var localX = worldPos.x - mapWorldPos.x;
          var localY = worldPos.y - mapWorldPos.y; // 考虑地图的缩放

          var mapScale = this.trackLayer.scale;
          var scaledLocalX = localX / mapScale.x;
          var scaledLocalY = localY / mapScale.y; // 转换为tile坐标

          var tileX = Math.floor(scaledLocalX / this._tileWidth);
          var tileY = Math.floor(scaledLocalY / this._tileHeight); // 注意：tilemap的Y轴是倒置的

          var mapY = this._mapHeight - 1 - tileY;
