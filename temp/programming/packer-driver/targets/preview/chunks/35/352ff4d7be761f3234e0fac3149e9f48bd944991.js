    ...
    execute: function () {
      ...
      _export("HealthBar", HealthBar = (_dec = ccclass('HealthBar'), _dec2 = property(ProgressBar), _dec(_class = (_class2 = class HealthBar extends Component {
        ...
        updateWithCameraConversion() {
          ...
          var worldPos = this.targetNode.worldPosition;
          var screenPos = this.camera.worldToScreen(worldPos); // 步骤2：将屏幕坐标转换为Canvas的本地坐标
          ...
          var localPos = canvasTransform.convertToNodeSpaceAR(new Vec3(screenPos.x, screenPos.y, 0)); // 步骤3：设置血条位置（在AI车辆上方）
        ...
        updateWithDirectFollow() {
          // 直接使用目标节点的位置，加上偏移
          var targetPos = this.targetNode.worldPosition;
