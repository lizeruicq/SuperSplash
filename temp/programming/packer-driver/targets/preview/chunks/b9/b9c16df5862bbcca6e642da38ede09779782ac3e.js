      ...
      _export("SelectManager", SelectManager = (_dec = ccclass('SelectManager'), _dec2 = property(ToggleContainer), _dec3 = property(ToggleContainer), _dec4 = property(Button), _dec5 = property(Button), _dec6 = property(Label), _dec7 = property({
        ...
        autoFindCarPropertyDisplay() {
          if (!this.carPropertyDisplay) {
            ...
            var carPropertyNode = find('Canvas/car-property') || find('car-property') || this.node.getChildByName('car-property');
        ...
        onCarToggleChanged(toggle) {
          if (toggle.isChecked) {
            var _carId = toggle.node.name;
            console.log("\u9009\u4E2D\u8F66\u8F86: " + _carId);
            this.showCarProperties(_carId);
          }
        }
        /**
         * 显示车辆属性
        ...
        checkInitialCarSelection() {
          var selectedToggle = this.carToggleGroup.toggleItems.find(toggle => toggle.isChecked);

          if (selectedToggle) {
            var _carId2 = selectedToggle.node.name;
            this.showCarProperties(_carId2);
          } else {
            // 如果没有选中的车辆，隐藏属性显示
            if (this.carPropertyDisplay) {
