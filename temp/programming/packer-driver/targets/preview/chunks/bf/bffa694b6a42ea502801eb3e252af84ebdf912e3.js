    ...
    execute: function () {
      ...
      _export("AIPlayer", AIPlayer = (_dec = ccclass('AIPlayer'), _dec2 = property(ProgressBar), _dec3 = property(SpriteFrame), _dec4 = property(Prefab), _dec5 = property({
        ...
        checkAndShoot() {
          ...
          var playerComponent = gameManager.getPlayerComponent();
          if (!playerComponent || !playerComponent.node) return; // 获取玩家和AI的位置

          var playerPos = playerComponent.node.getWorldPosition();
          var aiPos = this.node.getWorldPosition(); // 计算玩家相对于AI的方向向量

          var toPlayer = new Vec2(playerPos.x - aiPos.x, playerPos.y - aiPos.y); // 计算玩家相对于AI的角度

          var angleToPlayer = Math.atan2(toPlayer.y, toPlayer.x) * 180 / Math.PI; // 获取AI车辆的正前方角度（AI的角度+90度，因为车辆默认朝向是-90度）

          var aiForwardAngle = this._angle + 90; // 计算角度差

          var angleDiff = angleToPlayer - aiForwardAngle; // 将角度差标准化到-180到180度范围内
        ...
        shoot() {
          ...
          var offsetDistance = 60; // 子弹发射偏移距离（像素）
