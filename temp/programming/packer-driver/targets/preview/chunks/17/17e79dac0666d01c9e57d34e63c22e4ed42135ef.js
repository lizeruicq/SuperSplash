      ...
      _export("VehicleDestructionTest", VehicleDestructionTest = (_dec = ccclass('VehicleDestructionTest'), _dec2 = property(Label), _dec3 = property(Label), _dec4 = property(Label), _dec5 = property(Button), _dec6 = property(Button), _dec7 = property(Button), _dec8 = property(Button), _dec9 = property(Button), _dec(_class = (_class2 = class VehicleDestructionTest extends Component {
        ...
        findVehicles() {
          ...
          if (gameManager) {
            ...
            if (scene) {
              var playerNodes = scene.getComponentsInChildren(_crd && player === void 0 ? (_reportPossibleCrUseOfplayer({
                error: Error()
              }), player) : player);
        ...
        damageRandomAI(damage) {
          var aliveAIs = this.aiVehicles.filter(ai => !ai.isDestroyed());

          if (aliveAIs.length > 0) {
            var randomAI = aliveAIs[Math.floor(Math.random() * aliveAIs.length)];
        ...
        updateUI() {
          ...
          if (this.playerHealthLabel && this.playerVehicle) {
            var health = this.playerVehicle.getCurrentHealth();
            var maxHealth = this.playerVehicle.getMaxHealth();
            var status = this.playerVehicle.isDestroyed() ? ' [已摧毁]' : '';
            this.playerHealthLabel.string = "\u73A9\u5BB6\u8840\u91CF: " + health + "/" + maxHealth + status;
          } // 更新AI血量显示


          if (this.aiHealthLabel) {
            var aliveCount = this.aiVehicles.filter(ai => !ai.isDestroyed()).length;
            var totalCount = this.aiVehicles.length;
            this.aiHealthLabel.string = "AI\u8F66\u8F86: " + aliveCount + "/" + totalCount + " \u5B58\u6D3B";
          } // 更新状态显示


          if (this.statusLabel) {
            var playerDestroyed = this.playerVehicle ? this.playerVehicle.isDestroyed() : false;
            var allAIDestroyed = this.aiVehicles.length > 0 && this.aiVehicles.every(ai => ai.isDestroyed());
            var _status = '游戏进行中';
            ...
            this.statusLabel.string = "\u72B6\u6001: " + _status;
          }
        }
...
//# sourceMappingURL=17e79dac0666d01c9e57d34e63c22e4ed42135ef.js.map